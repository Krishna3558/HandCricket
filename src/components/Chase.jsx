...
function Chase() {
    ...
    let [tarComment , setTarComment] = useState('Click on calculate target to calculate the target');
    ...
    const targGet = () => {
        ...
        setTarget(x+1);
        setIsCal(true);
        setTarComment("Click on revise target to revise the target or play to chase the target");
        setTarDone(true);
    }
  ...
  return (
    ...
    {(sel) ? (<div className=' flex justify-center space-x-5'>
    <button onClick={targGet} disabled = {isCal} className=' bg-blue-900 text-white py-2 px-2 rounded-md font-semibold active:bg-blue-950 max-sm:px-2 max-sm:py-1 '>Calculate Target</button>
    <button onClick={targGet} disabled = {tar} className=' bg-blue-900 text-white py-2 px-2 rounded-md font-semibold active:bg-blue-950 max-sm:px-2 max-sm:py-1 '>Revise Target</button>
    </div>) : null}
    {(sel && !tar)  ? (<div className=' text-center font-semibold text-lg mt-6 max-sm:text-base'>{tarComment}</div>) : null}
    ...
    { tar ? (<nav className=' list-none bg-blue-900 flex justify-center space-x-32 max-lg:space-x-20 max-md:space-x-10  max-sm:space-x-6 max-sm:h-12 max-[356px]:space-x-2 font-extrabold text-lg text-white uppercase'>
        ...
        <li className=' py-4 max-sm:text-xs'>{you} Score: {currScore}</li>
